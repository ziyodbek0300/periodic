...
describe('Testing Auth', () => {
  describe('[POST] /signup', () => {
    it('response should have the Create userData', async () => {
      ...
      const admins = authRoute.authController.authService.admins;

      admins.findOne = jest.fn().mockReturnValue(null);
      admins.create = jest.fn().mockReturnValue({
  ...
  describe('[POST] /login', () => {
    it('response should have the Set-Cookie header with the Authorization token', async () => {
      ...
      admins.findOne = jest.fn().mockReturnValue({
        _id: '60706478aad6c9ad19a31c84',
        username: userData.username,
        password: await bcrypt.hash(userData.password, 10),
      });
  ...
  //     admins.findOne = jest.fn().mockReturnValue(userData);
